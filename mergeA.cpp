...
TFile *f1 = new TFile(path+"/PAMELA_LxPgModel/OUT/A_vs_time_Potgeiter.root");
f1->cd();
TGraphErrors *graph1 = (TGraphErrors*)f1->Get("grAvsTime_PAMELA");
...
TFile *f2 = new TFile(path+"/AMS02_NewSet_LxPgModel/OUT/A_vs_time_Potgeiter.root");
f2->cd();
TGraphErrors *graph2 = (TGraphErrors*)f2->Get("grAvsTime_AMS02");
...
TFile *f3 = new TFile(path+"BESS_LxPgModel/OUT/A_vs_time_PowerLaw.root");
f3->cd();
TGraphErrors *graph3 = (TGraphErrors*)f3->Get("grAvsTime_BESS");
...
TFile *f4 = new TFile(path+"SOHO_LxPgModel/OUT/A_vs_time_PowerLaw.root");
f4->cd();
TGraphErrors *graph4 = (TGraphErrors*)f4->Get("grAvsTime_SOHO");
graph4->SetMarkerStyle(1);
graph4->SetMarkerColor(kViolet+1);
graph4->SetLineColor(kViolet+1);
...
TFile *f5 = new TFile("A_Potgeiter.root","RECREATE");
c1->Write();
graph1->Write();
graph2->Write();
graph3->Write();
