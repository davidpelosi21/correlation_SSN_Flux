...
// --------- NTFit -----------
// -- Make K0 e N0 -Fits to CR proton data: -SOHO
// ---------------------------------
...
static const int nTimeSOHO_Proton      = 20; //81
const int iExp_SOHO       = 6;
...
// ---- external objects ----
NTSOHOData *sohodata;
NTPAMELAData* pameladata;  //def puntantore alla classe sohodata
...
TGraphErrors* grAvsTime_SOHO;
TGraphErrors* grBvsTime_SOHO;
TGraphErrors* grRkvsTime_SOHO;
TGraphErrors* grChis2vsTime_SOHO;
...
int main(int argc, char **argv){
   ...
   pameladata= new NTPAMELAData();
    sohodata = new NTSOHOData();

    sohodata->SetProtonData();
    ...
    grAvsTime_SOHO= new TGraphErrors();
    
    SetGraphAStyle();

    grBvsTime_SOHO= new TGraphErrors();
    
    SetGraphBStyle();

    grRkvsTime_SOHO= new TGraphErrors();
    SetGraphRkStyle();

    grChis2vsTime_SOHO= new TGraphErrors();
      ...
      //if( tt == 0 ) {
          ...
          if(Kmodel == 1){
           ...
           if(Kmodel == 3){
            ...
            //gminuit print the correlation matrix
            gMinuit->mnmatu(1);
            ...
            //cout <<" iTime: "<<tt<<"   K0= " << bestK0 << " +- "<<errK0<<" | "<<"    N0: "<<bestN0<< " +- "<<errN0 <<"     Chi2: "<<bestChi2_SOHO[tt]<< " / "<<NDF_SOHO[tt]<<endl;
              cout <<" iTime: "<<tt<<"   K0= " << bestK0 << " +- "<<errK0<<" | "<<"    a: "<<besta<< " +- "<<erra <<" | b: "<<bestb<<" +- "<<errb<<" | rk: "<<bestrk<<" +- "<<errrk<<" |    Chi2: "<<bestChi2_SOHO[tt]<< " / "<<NDF_SOHO[tt]<<endl;
            
          
            // ---- put results into graph ----
            grK0vsTime_SOHO->SetPoint(indTime, sohodata->xTimeSOHO_ProtonFlux[tt], bestK0);
            grK0vsTime_SOHO->SetPointError(indTime, sohodata->eTimeSOHO_ProtonFlux[tt], errK0);
            
            grAvsTime_SOHO->SetPoint(indTime, sohodata->xTimeSOHO_ProtonFlux[tt], besta);
            grAvsTime_SOHO->SetPointError(indTime, sohodata->eTimeSOHO_ProtonFlux[tt], erra);
          

            grBvsTime_SOHO->SetPoint(indTime, sohodata->xTimeSOHO_ProtonFlux[tt], bestb);
            grBvsTime_SOHO->SetPointError(indTime, sohodata->eTimeSOHO_ProtonFlux[tt], errb);
            

            grRkvsTime_SOHO->SetPoint(indTime, sohodata->xTimeSOHO_ProtonFlux[tt], bestrk);
            grRkvsTime_SOHO->SetPointError(indTime, sohodata->eTimeSOHO_ProtonFlux[tt], errrk);
            
            grChis2vsTime_SOHO->SetPoint(indTime, sohodata->xTimeSOHO_ProtonFlux[tt], bestChi2_SOHO[tt]/NDF_SOHO[tt]);
            grChis2vsTime_SOHO->SetPointError(indTime, sohodata->eTimeSOHO_ProtonFlux[tt], 0);
          
            
            
            indTime++; // increment time index
             
             //per plottare dovrei costruire il file root
...
void FCN(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag){
    ...
    // iExperiment: PAMELA / SOHO / BESS / BESSTeV / BESS00 / SOHO / SOHOPRL2015
...
double GetChiSquare(SolarModulation* NTSolution ,double K0, int iTime, int iExperiment, double N0,double a,double b,double rk) {
  ...
  // --- EXP6: SOHO ----
    if( iExperiment==iExp_SOHO){
        NDF_SOHO[iTime]=0;
        for(int ee=0;ee<sohodata->nEknSOHO_Proton;ee++){ // ekn loop
            // get DATA
            double Energy= sohodata->grSOHO_ProtonFluxVSEkn[iTime]->GetX()[ee];
            if(Energy<Emin || Energy>Emax) continue;  // Check Range
            
            double Ekn   = (1.e+3)*Energy;  // convert GeV->MeV
            double Flux  = (1.e-3)*sohodata->grSOHO_ProtonFluxVSEkn[iTime]->GetY()[ee];
            double eFlux = (1.e-3)*sohodata->grSOHO_ProtonFluxVSEkn[iTime]->GetEY()[ee];
...
*/
  ...
  grK0vsTime_SOHO->SetName("grK0VSTime_SOHO");
  grK0vsTime_SOHO->SetMarkerStyle( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerStyle() );
  grK0vsTime_SOHO->SetMarkerColor( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerColor() );
  grK0vsTime_SOHO->SetMarkerSize( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerSize() );
  grK0vsTime_SOHO->SetLineWidth( sohodata->grSOHO_ProtonFluxVSTime[0]->GetLineWidth() );
  grK0vsTime_SOHO->SetLineColor( sohodata->grSOHO_ProtonFluxVSTime[0]->GetLineColor() );
  SetTimeGraphStyle(grK0vsTime_SOHO);
...
*/
  ...
  grAvsTime_SOHO->SetName("grAvsTime_SOHO");

  grAvsTime_SOHO->SetMarkerStyle( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerStyle() );
  grAvsTime_SOHO->SetMarkerColor( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerColor() );
  grAvsTime_SOHO->SetMarkerSize( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerSize() );
  grAvsTime_SOHO->SetLineWidth( sohodata->grSOHO_ProtonFluxVSTime[0]->GetLineWidth() );
  grAvsTime_SOHO->SetLineColor( sohodata->grSOHO_ProtonFluxVSTime[0]->GetLineColor() );
  SetTimeGraphStyle(grAvsTime_SOHO);
  grAvsTime_SOHO->GetYaxis()->SetTitle("a");
...
void SetGraphBStyle(){
  ...
  grBvsTime_SOHO->SetName("grBvsTime_SOHO");

  grBvsTime_SOHO->SetMarkerStyle( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerStyle() );
  grBvsTime_SOHO->SetMarkerColor( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerColor() );
  grBvsTime_SOHO->SetMarkerSize( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerSize() );
  grBvsTime_SOHO->SetLineWidth( sohodata->grSOHO_ProtonFluxVSTime[0]->GetLineWidth() );
  grBvsTime_SOHO->SetLineColor( sohodata->grSOHO_ProtonFluxVSTime[0]->GetLineColor() );
  SetTimeGraphStyle(grBvsTime_SOHO);
  grBvsTime_SOHO->GetYaxis()->SetTitle("b");
...
void SetGraphRkStyle(){
  ...
  grRkvsTime_SOHO->SetName("grRkvsTime_SOHO");

  grRkvsTime_SOHO->SetMarkerStyle( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerStyle() );
  grRkvsTime_SOHO->SetMarkerColor( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerColor() );
  grRkvsTime_SOHO->SetMarkerSize( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerSize() );
  grRkvsTime_SOHO->SetLineWidth( sohodata->grSOHO_ProtonFluxVSTime[0]->GetLineWidth() );
  grRkvsTime_SOHO->SetLineColor( sohodata->grSOHO_ProtonFluxVSTime[0]->GetLineColor() );
  SetTimeGraphStyle(grRkvsTime_SOHO);
  grRkvsTime_SOHO->GetYaxis()->SetTitle("Rk");
...
void SetGraphChis2Style(){
  ...
  grChis2vsTime_SOHO->SetName("grChis2vsTime_SOHO");

  grChis2vsTime_SOHO->SetMarkerStyle( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerStyle() );
  grChis2vsTime_SOHO->SetMarkerColor( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerColor() );
  grChis2vsTime_SOHO->SetMarkerSize( sohodata->grSOHO_ProtonFluxVSTime[0]->GetMarkerSize() );
  grChis2vsTime_SOHO->SetLineWidth( sohodata->grSOHO_ProtonFluxVSTime[0]->GetLineWidth() );
  grChis2vsTime_SOHO->SetLineColor( sohodata->grSOHO_ProtonFluxVSTime[0]->GetLineColor() );
  SetTimeGraphStyle(grChis2vsTime_SOHO);
  grChis2vsTime_SOHO->GetYaxis()->SetTitle("#chi^{2}/d.o.f.");
