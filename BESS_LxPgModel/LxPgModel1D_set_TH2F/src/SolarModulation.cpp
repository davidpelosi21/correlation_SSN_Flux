...
TString PATH = "/Users/davidpelosi/Desktop/TESI-Master/LxPgModel1D_set_TH2F";
...
//SolarModulation::SolarModulation(double Z, double A, int LIS, int iXiD, int Kmodel){
  pZ = Z;
  pA = A;
  pM = ProtonMass*A;
  kLISModel = LIS;
...
void SolarModulation::Solve(){
  ...
  for(int j=np-2; j>=0; --j){ //loop on rigidity j
    ...
    Vec B(nr-2, aux);
      ...
      EqSolver* S = new EqSolver(*(PA[j]),B);
...
void SolarModulation::Iterate_ThisKScale(int iKScale){ // Grid gia definite
    indexKScale= iKScale;
