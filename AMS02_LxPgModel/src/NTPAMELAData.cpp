...
void NTPAMELAData::SetProtonData(){
  ...
  //TFile* inFilePAMELA= new TFile(PATH_PAMELA+"/data/grPAMELA_Proton_VS_Ekn_VS_Time.root","READ");
   TFile* inFilePAMELA= new TFile(PATH_PAMELA+"/data/ssdc_canvas.root","READ");
...
inFilePAMELA->ls();
  ...
  for(int tt=0;tt<nTimePAMELA_Proton;tt++){
      ...
      grPAMELA_ProtonFluxVSEkn[tt]=(TGraphAsymmErrors*)inFilePAMELA->Get(Form("graph%d",tt+1));
  ...
  cout<<"pamela PROTON EKN.. SPECTRA DONE!"<<endl;
  

  TFile* inFilePAMELA_time= new TFile(PATH_PAMELA+"/data/pamela_Flux_time_merged.root","READ");

  // ---- get fluxes vs time ---- | NT BUT they are RIGIDITY fluxes!!
  for(int ee=0;ee<nEknPAMELA_Proton;ee++){
    grPAMELA_ProtonFluxVSTime[ee]=(TGraphErrors*)inFilePAMELA_time->Get("MergedGraph");  
