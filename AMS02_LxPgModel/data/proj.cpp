...
TH2F *h3= (TH2F*)f->Get("Monthly_Stat_errors_Rebin");
TH2F *h2= (TH2F*)f->Get("Monthly_TIndep_Syst_errors_Rebin");

//for each time bin

TH1F *flux[h->GetNbinsX()];
TH1F *rel_stat_err[h2->GetNbinsX()];
TH1F *rel_sys_err[h3->GetNbinsX()];
...
for (int i=0; i<h2->GetNbinsX(); i++) {
  rel_stat_err[i] = (TH1F*)h2->ProjectionY(Form("rel_sys_err_N%d",i),i+1,i+1);
}

for (int i=0; i<h3->GetNbinsX(); i++) {
  rel_sys_err[i] = (TH1F*)h3->ProjectionY(Form("rel_sys_err_N%d",i),i+1,i+1);
}
...
TH1F *rel_sys_err_R[h3->GetNbinsY()];
...
for (int i=0; i<h3->GetNbinsY(); i++) {
  rel_sys_err_R[i] = (TH1F*)h3->ProjectionX(Form("R_rel_sys_err_N%d",i),i+1,i+1);
}
...
for (int j=0; j<h->GetNbinsX(); j++) {
   ...
   for (int i=0; i<h->GetNbinsY(); i++) {
      //ekn
   /*   flux_err_stat[j].SetPoint(i,RigToEkn(flux[j]->GetBinCenter(i+2)), flux[j]->GetBinContent(i+2)*dRdE(RigToEkn(flux[j]->GetBinCenter(i+2))) );
      flux_err_stat[j].SetPointError(i,0,RigToEkn(  flux[j]->GetBinContent(i+2)*rel_stat_err[j]->GetBinContent(i+2)) );
   */

   //rig
   flux_err_stat[j].SetPoint(i,flux[j]->GetBinCenter(i+2),flux[j]->GetBinContent(i+2));
   flux_err_stat[j].SetPointError(i,0,flux[j]->GetBinContent(i+2)*rel_stat_err[j]->GetBinContent(i+2));
...
for (int j=0; j<h->GetNbinsY(); j++) {
   ...
   for (int i=0; i<114; i++) {
     ...
     flux_err_stat_R[j].SetPoint(i,flux_R[j]->GetBinCenter(i+2), flux_R[j]->GetBinContent(i+2) );
     flux_err_stat_R[j].SetPointError(i,0,flux_R[j]->GetBinContent(i+2)*rel_stat_err_R[j]->GetBinContent(i+2));
   }
}
