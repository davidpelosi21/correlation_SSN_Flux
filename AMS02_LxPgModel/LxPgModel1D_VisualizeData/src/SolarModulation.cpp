...
TString PATH = "/Users/davidpelosi/Desktop/TESI-Master/LxPgModel1D_VisualizeData";

// ---- NT Constructor ---
SolarModulation::SolarModulation(double Z, double A, int LIS, int RID){
  ...
  // ---- terminate run for bad inputs ----
  if(DRIFT==0 && RunID>=NK){ 
    cout<<"RunID must be between 0 and "<<NK-1<<". Program terminated ¯\\_(ツ)_/¯"<<endl; 
    exit(1); 
  }
  if(DRIFT==1 && RunID>=NX*NK){ 
    cout<<"RunID must be between 0 and "<<NX*NK-1<<". Program terminated ¯\\_(ツ)_/¯"<<endl; 
    exit(1);
  }

  // ---- printout header ----
  printf("\n");
  printf("******* Run N. %d | LIS Model %d | Particle Z%d A%d ********\n\n",RunID, kLISModel, (int)Z, (int)A);
  if(DRIFT==0)printf("Drift OFF\n");
  printf("Input parameter... RunID: %d\n",RunID);
  printf("Corresponding indices: iK0: %d   iXiD: %d \n",indexKScale, indexXiD);
  printf("\n");
  printf("Numerical grid with parameters:\n");
  printf("==> p = [%lf,%lf] with %d points\n", pmin, pmax, np);
  printf("==> r = [%lf,%lf] with %d points\n\n", rmin, rmax, nr);

  // --- build grids & TF1s ---
  InitModulation();    
  MakeGridsForHistograms();     
}


// ---- NT Constructor ---
SolarModulation::SolarModulation(double Z, double A, int LIS, int iKScale, int iXiD, int Kmodel){
  pZ = Z;
  pA = A;
  pM = ProtonMass*A;
  kLISModel = LIS;
  ...
  // ---- printout header ----
  printf("\n");
  printf("******* Run with iK0: %d   iXD: %d | LIS Model %d | Particle Z %d A %d ********\n\n",indexKScale, indexXiD, kLISModel, (int)Z, (int)A);

  printf("Input parameters... iK0: %d   iXiD: %d \n",indexKScale, indexXiD);
  printf("Corresponding Run N.: %d\n",RunID);
  printf("Numerical grid with parameters:\n");
  printf("==> p = [%lf,%lf] with %d points\n", pmin, pmax, np);
  printf("==> r = [%lf,%lf] with %d points\n\n", rmin, rmax, nr);
...
/*
SolarModulation::SolarModulation(double Z, double A, int LIS, int iKScale, int iXiD){
  pZ = Z;
  pA = A;
  pM = ProtonMass*A;
  kLISModel = LIS;
...
void SolarModulation::InitModulation(){
    ...
    for(int kk=0; kk<nk; ++kk) lnKScale[kk] = log(kmin) + deltalogk*kk;
...
SolarModulation::~SolarModulation(){   //distruttore
  ...
  delete hLIS_vs_Rigidity;
  delete hLIS_vs_KEnergy;
  delete hFlux_vs_Rigidity;
  delete hFlux_vs_KEnergy;
  delete hFlux_vs_Radius;
  delete hFlux_vs_KScale;
  
  delete hFluxFF_vs_Rigidity;
  delete hFluxFF_vs_KEnergy;
  delete hFluxFF_vs_Phi_vs_Rigidity;
  delete hFluxFF_vs_Phi_vs_KEnergy;
  
  delete hFlux_vs_KScale_vs_Rigidity;
  delete hFlux_vs_KScale_vs_KEnergy;
  delete hFlux_vs_Radius_vs_Rigidity;
  delete hFlux_vs_Radius_vs_KEnergy;

  delete hLn_Flux_vs_KScale_vs_Rigidity;
  delete hLn_Flux_vs_KScale_vs_KEnergy;
  delete hLn_FluxFF_vs_Phi_vs_Rigidity;
  delete hLn_FluxFF_vs_Phi_vs_KEnergy;

  delete hFlux_vs_XiD_vs_KScale_vs_Rigidity;
  delete hFlux_vs_XiD_vs_KScale_vs_KEnergy;
  delete hLn_Flux_vs_XiD_vs_KScale_vs_Rigidity;
  delete hLn_Flux_vs_XiD_vs_KScale_vs_KEnergy;
...
void SolarModulation::MakeGridsForHistograms(){
  ...
  // ---- some printout ----
  // for(int rr=0;rr<nRadius+1;rr++){
  //   cout<<rr<<"   RADIUS: "<<xRadius[rr]/AU <<endl;
  // } 

  // cout<<endl;
  // for(int pp=0;pp<nRigidity+1;pp++){
  //   cout<<pp<<"   Rigidity: "<<xRigidity[pp] <<"     KEnergy: "<<xKEnergy[pp]<<endl;
  // } 
...
void SolarModulation::InitJLis(){
  ...
  if( kLISModel>0){
    ...
    //    if( kLISModel==4) inLIS= new TFile("$NTBASEDIR/THEORY/00matisse/PHeModel/input/grLIS_pHeBCO_Reina_2020.root","READ");
   ...
   // cout<<"CONversion 1 GV ---> "<<pow(RigToEkn(1.e3),-1)<<"  MeV"<<endl;
    if (kLISModel < 4) {
      ...
      for(int ee=0;ee<nLIS;ee++){
        ...
        cout<<"E (MeV/n)= "<<exp(xLnEkn[2])<<endl;
        cout<<"flux (MeV^{-1})= "<<exp(xLnLIS[2])<<endl;
...
void SolarModulation::SetKCoeff(){
  ...
  if(indexKScale>-1) KCoeff->SetParameter(0, ThisKScale);  // useless
}
...
void SolarModulation::Solve(){
  ...
  for(int i=0; i<nr; ++i) f[i][np-1] = (JLisT->Eval(Tmax)/(pmax*pmax*NTF))/f0;  //valori dei nodi della griglia psi al contorno
    //f è la psi,
  // NT Should an pA/pZ factor be here?
  ...
  for(int j=np-2; j>=0; --j){ //loop on rigidity j
    ...
    Vec B(nr-2, aux);
    EqSolver S(*(PA[j]),B);
    Vec sol = S.RecursiveTriDiagonal();
...
void SolarModulation::SetSolutions(){
  ...
  if(indK0>=0 && indK0<nKScale){
    ...
    for(int ee=0;ee<nKEnergy;ee++){
      ...
      hLn_Flux_vs_KScale_vs_KEnergy->SetBinContent(ee+1, indK0+1, log(pFluxEkn));
    }
    ...
    for(int pp=0;pp<nRigidity;pp++){
      ...
      hLn_Flux_vs_KScale_vs_Rigidity->SetBinContent(pp+1, indK0+1, log(pFluxRig));
    }
...
void SolarModulation::StoreResults(){
  ...
  if(DRIFT==0){ 
    ...
    outFile= new TFile(Form(PATH+"/OUT/PHeModel_DriftOFF/hrun_BetaOFF_DriftOFF_FluxResults_LIS%d_Z%d_A%d_RunID%d.root",kLISModel,zz,aa,RunID),"RECREATE");
  }
  ...
  if(DRIFT==1){ // kk and dd indices
    ...
    outFile= new TFile(Form(PATH+"/OUT/PHeModel_Z%d_A%d/hrun_FluxResults_LIS%d_Z%d_A%d_XiD%d_K%d_RunID%d.root",zz,aa,kLISModel,zz,aa,dd,kk,RunID),"RECREATE");
  }
...
// sono i dati dentro in file Voyager in data
  ...
  for(int ee=0;ee<16;ee++){
    ...
    /*
    TFile* inFileHelium= new TFile(PATH+"/data/grVOYAGER1_pHe4_Spectra_Ekn_Rig.root","READ");
    inFileHelium->cd();
    TGraphAsymmErrors* grVoyager1_He3_Flux= (TGraphAsymmErrors*)inFileHelium->Get("grVoyager1_Helium4_Flux_Ekn");
    grVoyager1_He3_Flux->SetMarkerStyle(20);
    grVoyager1_He3_Flux->SetMarkerColor(kBlack);
    grVoyager1_He3_Flux->SetMarkerSize(0.8);
    inFileHelium->Close();

    // --- GeV->MeV conversion ---

    
    for(int ee=0;ee<grVoyager1_He3_Flux->GetN();ee++){
        grVoyager1_He3_Flux->GetX()[ee] *= 1.e+3;
        grVoyager1_He3_Flux->GetY()[ee] *= 1.e-3;
        //grVoyager1_He3_Flux->GetEXlow()[ee] *= 1.e+3;
        //grVoyager1_He3_Flux->GetEXhigh()[ee] *= 1.e+3;
        grVoyager1_He3_Flux->GetEY()[ee] *= 1.e-3;
   ...
   grVOYAGER1_Hydrogen_Flux_2016->Draw("pZ");
    ...
    TCanvas* ccKCoeff = new TCanvas(Form("ccKCoeff"), Form("K vs Rigidity - k0=%f 10^{18}",ThisKScale), 600, 600); //1400, 1000);
    ccKCoeff->cd();
    ...
    // store locally
    TFile* outFile;
    outFile= new TFile(Form(PATH+"/OUT/TGraph_Flux_data_LIS%d_Z%d_A%d_RunID%d.root",kLISModel,zz,aa,RunID),"recreate");
    outFile->cd();
...
void SolarModulation::PlotSolution(){
  ...
  for(int ee=0;ee<16;ee++){
    ...
    TCanvas* ccFluxSolution = new TCanvas(Form("ccFluxSolution_Z%d_A%d",zz,aa), Form("SOLUTION for Z=%d A=%d",zz,aa), 600, 600); //1400, 1000);
    ccFluxSolution->cd();
    gPad->SetBottomMargin(0.18);
    gPad->SetLeftMargin(0.18);
    gPad->SetRightMargin(0.05);
    ...
    // grVOYAGER1_Helium_Flux_2016->Draw("pZ");
    grAMS02_Proton_Flux_2015->Draw("pZ");
    // // grAMS02_Helium_Flux_2015->Draw("pZ"); // METTERE...?
...
*/
   ...
   // --- GeV->MeV conversion ---
    cout<<"Conversione12335-----"<<endl;
    
   for(int ee=0;ee<grAMS02_Helium_Flux->GetN();ee++){
       grAMS02_Helium_Flux->GetX()[ee] = EknToRig(grAMS02_Helium_Flux->GetX()[ee]*1.e+3);
       grAMS02_Helium_Flux->GetY()[ee] =   grAMS02_Helium_Flux->GetY()[ee]*pow(EknToRig(1.e+3),-1);
       //grAMS02_Helium_Flux->GetEXlow()[ee] *= 1.e+3;
       //grAMS02_Helium_Flux->GetEXhigh()[ee] *= 1.e+3;
       grAMS02_Helium_Flux->GetEY()[ee] = grAMS02_Helium_Flux->GetEY()[ee]*pow(EknToRig(1.e+3),-1);
       //grAMS02_Helium_Flux->GetEYhigh()[ee] *= 1.e-3;
   }
    ...
    // ---- get AMS02 data vs Ekn ----
    TFile* inFileProtonAMS= new TFile(PATH+"/data/grProtonData.root","READ");
    inFileProtonAMS->cd();
    cout<<"Conversione*******-----"<<endl;
    
    TGraphAsymmErrors* grAMS02_Proton_Flux_2015 = (TGraphAsymmErrors*)inFileProton->Get("gr_exp1");
    cout<<"Conversione*******-----"<<endl;
    cout<<"Conversione*******00000000000-----"<<endl;
    
    //grAMS02_Proton_Flux_2015->SetMarkerStyle(20);
    //grAMS02_Proton_Flux_2015->SetMarkerColor(kBlack);
    //grAMS02_Proton_Flux_2015->SetMarkerSize(0.8);
    inFileProtonAMS->Close();

    cout<<"Conversione*******1111111111111111-----"<<endl;
    // --- GeV->MeV conversion ---

    cout<<"grAMS02_Proton_Flux_2015-----"<<endl;
    
    for(int ee=0;ee<grAMS02_Proton_Flux_2015->GetN();ee++){
        cout<<"grAMS02_Proton_Flux_2015-----"<<endl;
        grAMS02_Proton_Flux_2015->GetX()[ee] = EknToRig(grAMS02_Proton_Flux_2015->GetX()[ee]*1.e+3);
        grAMS02_Proton_Flux_2015->GetY()[ee] =   grAMS02_Proton_Flux_2015->GetY()[ee]*pow(EknToRig(1.e+3),-1);
        
        grAMS02_Proton_Flux_2015->GetEXlow()[ee] = EknToRig(grAMS02_Proton_Flux_2015->GetEXlow()[ee]*1.e+3);
        grAMS02_Proton_Flux_2015->GetEXhigh()[ee] = EknToRig(grAMS02_Proton_Flux_2015->GetEXhigh()[ee]*1.e+3);
        
        grAMS02_Proton_Flux_2015->GetEYlow()[ee] =   grAMS02_Proton_Flux_2015->GetEYlow()[ee]*pow(EknToRig(1.e+3),-1);
        grAMS02_Proton_Flux_2015->GetEYhigh()[ee] =   grAMS02_Proton_Flux_2015->GetEYhigh()[ee]*pow(EknToRig(1.e+3),-1);
...
*/
    ...
    TFile* inFileHelium= new TFile(PATH+"/data/grVOYAGER1_pHe3_Spectra_Ekn_Rig.root","READ");
    inFileHelium->cd();

    
    TGraphAsymmErrors* grVoyager1_He_Flux= (TGraphAsymmErrors*)inFileHelium->Get("grVoyager1_Helium3_Flux_Rig");
    grVoyager1_He_Flux->SetMarkerStyle(20);
    grVoyager1_He_Flux->SetMarkerColor(kBlack);
    grVoyager1_He_Flux->SetMarkerSize(0.8);
    inFileHelium->Close();

    // --- GV->MV conversion ---
    
    for(int ee=0;ee<grVoyager1_He_Flux->GetN();ee++){
        
        grVoyager1_He_Flux->GetX()[ee] *= 1.e+3;
        grVoyager1_He_Flux->GetY()[ee] *= 1.e-3;
        //grAMS02_Helium_Flux->GetEXlow()[ee] *= 1.e+3;
        //grAMS02_Helium_Flux->GetEXhigh()[ee] *= 1.e+3;

        grVoyager1_He_Flux->GetEY()[ee] *= 1.e-3;
    ...
    TGraph* grFluxSolution= new TGraph(np, Rigidity, flux[indR0]);
    ...
    double PMIN=1.; // 10 MeV
    double PMAX=100000.; // 100 GeV

    TH2F* hFrameProtonEkn= new TH2F("hFrameProtonEkn","hFrameProtonEkn",200, pmin*1.e-3, pmax*1.e-3, 200, 1.e-6, 1.e+5); // MeV
    //TH2F* hFrameProtonEkn= new TH2F("hFrameProtonEkn","hFrameProtonEkn",300, emin, emax, 300, 1.e-7, 1.e4); // MeV


    hFrameProtonEkn->GetXaxis()->SetTitle("Rigidity [GV]");
    hFrameProtonEkn->GetYaxis()->SetTitle("J(E) [GV^{ -1} m^{ -2} s^{ -1} sr^{ -1} ]");
    hFrameProtonEkn->SetTitle(0);
    hFrameProtonEkn->GetYaxis()->SetNdivisions(504);
    hFrameProtonEkn->SetLabelFont(42,"X");
    hFrameProtonEkn->SetLabelFont(42,"Y");
    ...
    //grVoyager1_He_Flux->Draw("pZ");
    //grAMS02_Helium_Flux->Draw("pZ");
