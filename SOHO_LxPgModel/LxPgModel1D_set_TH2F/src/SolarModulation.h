...
class SolarModulation {
 ...
 public:
  ...
  SolarModulation(double Z, double A, int LIS, int RID);
  //SolarModulation(double Z, double A, int LIS, int iKScale, int iXiD);
  //SolarModulation(double Z, double A, int LIS, int iKScale, int iXiD, int Kmodel);
    SolarModulation(double Z, double A, int LIS,  int iKScale, int iXiD, int Kmodel);
  ...
  void InitModulation();
    ...
    void Iterate_ThisKScale(int iKScale);    // initialize LIS model
...
*/
    ...
    double functorDC(double* x, double* par){
      ...
      if(indexKScale>-1) Norm = (1.e+18)*ThisKScale; // SCALE HERE !!!
	...
	Beta*
	pow(P/P0,a)*
	pow(  ((pow(P/P0,c) + pow(Pk/P0,c))/( 1.+pow(Pk/P0,c) ) ), (b-a)/c  );
      return K;
    }
