...
void NTAMS02Data::SetProtonData(){
  ...
  // ---- pick up data files ----
//TFile* inFileAMS02= new TFile(PATH_02+"/data/AMS02.root","READ");
TFile* inFileAMS02= new TFile(PATH_02+"/data/AMS02_RES23.root","READ");
...
inFileAMS02->ls();
  ...
  cout<<"PROTON EKN.. SPECTRA DONE!"<<endl;
   ...
   TFile* inFileAMS02_1= new TFile(PATH_02+"/data/AMS02.root","READ");
   

     cout<<"PROTON EKN.. SPECTRA DONE!"<<endl;

  // ---- get fluxes vs time ---- | NT BUT they are RIGIDITY fluxes!!
  for(int ee=0;ee<nEknAMS02_Proton-2;ee++){
  grAMS02_ProtonFluxVSTime[ee]=(TGraphErrors*)inFileAMS02_1->Get(Form("grProton_vs_Time_R%d",ee+2));  
  ...
  // ---- get time array ---- 
  double* xTimeAMS02= (double*)grAMS02_ProtonFluxVSTime[1]->GetX();
  ...
  double* xEknAMS02= (double*)grAMS02_ProtonFluxVSEkn[1]->GetX();
