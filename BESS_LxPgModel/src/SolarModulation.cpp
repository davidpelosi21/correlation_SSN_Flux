...
// sono i dati dentro in file Voyager in data
  ...
  for(int ee=0;ee<16;ee++){
    ...
    inFileProton->cd();
    TGraphAsymmErrors* gr_Proton_Flux= (TGraphAsymmErrors*)inFileProton->Get("gr_exp1");
    gr_Proton_Flux->SetMarkerStyle(20);
    gr_Proton_Flux->SetMarkerColor(kBlack);
    gr_Proton_Flux->SetMarkerSize(0.8);
    inFileProton->Close();

    // --- GeV->MeV conversion ---
    for(int ee=0;ee<gr_Proton_Flux->GetN();ee++){
      gr_Proton_Flux->GetX()[ee] *= 1.e+3;
      gr_Proton_Flux->GetY()[ee] *= 1.e-3;
      gr_Proton_Flux->GetEXlow()[ee] *= 1.e+3;
      gr_Proton_Flux->GetEXhigh()[ee] *= 1.e+3;
      gr_Proton_Flux->GetEYlow()[ee] *= 1.e-3;
      gr_Proton_Flux->GetEYhigh()[ee] *= 1.e-3;
    }
...
void SolarModulation::PlotSolution_BR(int br){
  ...
  // ---- get ams data vs Ekn ----
    ...
    TFile* inFileProton= new TFile("./data/grBESS_ProtonFluxes.root","READ");
    
TGraphErrors* gr_Proton_Flux = new TGraphErrors();

if (br == 0) {
    inFileProton->cd();
     gr_Proton_Flux = (TGraphErrors*)inFileProton->Get("grBESS00_ProtonFlux_2007");
...
if (br == 1) {
    inFileProton->cd();
    gr_Proton_Flux = (TGraphErrors*)inFileProton->Get("grBESSTeV_ProtonFlux_2007");
  ...
  if (br == 2) {
    inFileProton->cd();
    gr_Proton_Flux = (TGraphErrors*)inFileProton->Get("grBESSPolarI_ProtonFlux_2016");
  ...
  if (br == 3) {
    inFileProton->cd();
    gr_Proton_Flux = (TGraphErrors*)inFileProton->Get("grBESSPolarII_ProtonFlux_2016");
