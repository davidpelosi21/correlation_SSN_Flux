...
//void EqSolver::SetConstants(const FCmatrixFull& A, const Vec& v)  {
void EqSolver::SetConstants(const FCmatrix& A, const Vec& v)  {
    // attributo banda non definto -> non serve per il metodo RecursiveTriDiagonal
    M->M.clear();
    vector<Vec> aux;
    aux = A.GetM();
    b = v;
...
Vec EqSolver::RecursiveTriDiagonal(){
   ...
   /* for (int i=0; i<n; ++i) alpha[i] = (*M)[i+1][i];
    ...
    for (int i=0; i<n; ++i) alpha[i] =  M->outvec(i+1,i);
    for (int i=0; i<n+1; ++i) beta[i] =  M->outvec(i,i);
    for (int i=0; i<n; ++i) gama[i] =  M->outvec(i,i+1);
